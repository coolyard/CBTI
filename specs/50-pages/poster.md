...
> 信息密度原则（PRD §九）：完整立绘 + 双台词 + 角色身份 + 解读首段 + 迷你雷达 + 小程序码，刚好一条朋友圈。
...
| 解读首段 | 标签下方 | `interpretation[0]` 折行，最多 3 行，超出以 `…` 截断 |
| 迷你雷达 | 解读下方自适应 | 复用 `drawRadar`（`animate: false`，隐藏图例与数值只留维度名），尺寸 150–220 |
| 底部条 | y 1228 起 | 左侧：静态小程序码 94×94；右侧文案「长按识别 · 测测你的灵魂角色」 |
...
- 文本换行：自实现 `wrapText(ctx, text, maxWidth)`（按字符宽度测量，中文逐字断行）；解读与台词共用折行/截断逻辑。
- 台词与解读都复用白底、3px ink 描边、圆角贴纸风；台词气泡位于立绘正上方，尾巴朝下。
...
- [ ] 台词超 30 字自动换行不破版；解读首段超 3 行以 `…` 截断
- [ ] 完整立绘全幅绘制不裁切；缺失时使用首字符 + 轮换底色占位不阻断导出
