...
CBTI（Character-Based Type Indicator）是一个纯前端的娱乐人格测试：答完 15 道选择题，生成 5 维灵魂数据指纹，用曼哈顿距离匹配 28 组灵魂原型，最终给你一张可截图、可分享的「灵魂角色身份证」。
...
- 答题页：15 道题每题一屏、250ms 自动进题、可回改、进度持久化
...
- 彩蛋：题 14/15 的隐藏选项可锁定魔童哪吒或黑神话孙悟空
...
1. 用户答 15 道题，每题 4 个选项对应 `L / M1 / M2 / H`
2. 每维度 3 题得分取均值，得到 5 维 `dimensionScore`
3. 均分定档生成 5 位模式串，例如 `H-H-M-L-H`
4. 与角色库模式串计算曼哈顿距离，取最近角色为主结果、次近为灵魂近亲
5. 题 14/15 的隐藏选项可触发彩蛋锁定
...
内容已回写 `src/data/characters.ts`，并在 `src/data/index.ts` 通过 `assertContentComplete()` 强制校验。

## 立绘与分包

- 54 张 WebP 立绘位于 `src/pkg-characters/characters/`
- 命名规则：`char-{原型两位补零}-{male|female|universal}.webp`
- 微信小程序按分包 `pkg-characters` 打包，当前总量约 1.83MB，低于 2MB 上限
- 加载页进入时在 MP 端预下载分包，H5 不执行
- 首页/结果页/海报均有「立绘失败回退首字符占位」的降级逻辑
